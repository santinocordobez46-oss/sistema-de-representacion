...
function renderTable() {
  ...
  const students = notasData.students.filter((s) => {
  ...
  thead.innerHTML = `<tr>
      <th>N° Alumno</th><th>Nombre</th><th>Comisión</th>
...
document.getElementById("btn-export-xlsx").addEventListener("click", () => {
  ...
  const forms = notasData.forms;
  const data = notasData.students.map((s) => {
    const row = { "N° Alumno": s.numeroAlumno, "Nombre": s.nombre, "Comisión": s.comision };
    let sumScore = 0, sumMax = 0;
    forms.forEach((f) => {
      const p = s.parciales[f.id];
